...
func removeSpaceAndTabs(data []byte) []byte {
...
func Decode(data []byte) (p *Block, rest []byte) {
	...
	for {
		...
		for {
			...
			_, next, consumed := getLine(rest)
			key, val, ok := bytes.Cut(rest, colon)
			if !ok {
				break
			}
		...
		p.Bytes = []byte{}
		if endIndex > 0 {
			base64Data := removeSpaceAndTabs(rest[:endIndex])
...
func Encode(out io.Writer, b *Block) error {
	...
	for k := range b.Headers {
		if strings.Contains(k, ":") {
			return errors.New("pem:canot encode a header key that contains a colon")
		}
	}
