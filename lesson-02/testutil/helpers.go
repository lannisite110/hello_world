...
func loadEnv() {
	envOnce.Do(func() {
		...
		fmt.Println("loadEnv currentFile=", currentFile)
		if !ok {
			return
		}
		...
		fmt.Println("loadEnv testutilDir=", testutilDir)
		exampleDir := filepath.Dir(testutilDir)
		fmt.Println("loadEnv exampleDir=", exampleDir)

		// Load .env file from examples directory
		envPath := filepath.Join(exampleDir, ".env")
		fmt.Println("loadEnv envPath=", envPath)
...
func getDBDir() (string, error) {
	...
	_, currentFile, _, ok := runtime.Caller(0)
	println("getDBDir currentFile=", currentFile)
	if !ok {
		return "", os.ErrInvalid
	}
	// Get the examples directory (parent of testutil)
	testutilDir := filepath.Dir(currentFile)
	println("getDBDir testutilDir=", testutilDir)
	exampleDir := filepath.Dir(testutilDir)
	println("getDBDir exampleDir=", exampleDir)
	// The db directory is examples/db
	dbDir := filepath.Join(exampleDir, "db")
	println("getDBDir dbDir=", dbDir)

	if err := os.MkdirAll(dbDir, 0755); err != nil {
		println("getDBDir err=", err)
		return "", err
	}
...
func NewTestDB(t *testing.T, filename string) *gorm.DB {
	println("NewTestDB Helper")
	t.Helper()
	println("NewTestDB getDBType")
	dbtype := getDBType()

	println("NewTestDB dbtype=", dbtype)
	...
	switch dbtype {
	case DBTypeSQLite:
		println("NewTestDB DBTypeSQLite=", DBTypeSQLite)
		db, err = newSQLiteDB(t, filename)
		println("NewTestDB filename=", filename)
	case DBTypeMYSQL:
		db, err = newMySQLDB(t)
	case DBTypePostgres:
...
func newSQLiteDB(t *testing.T, filename string) (*gorm.DB, error) {
	...
	dbDir, err := getDBDir()
	println("newSQLiteDB dbDir=", dbDir)
	if err != nil {
		return nil, err
	}
...
func newPostgresDB(t *testing.T) (*gorm.DB, error) {
	...
	if dsn == "" {
		dsn = "host=localhost user=postgres password=password dbname=testdb port=5432 sslmode=disable TimeZone=Asia/Shainghai"
