...
func main() {
	...
	if *rangeStartFlag > 0 && *rangeEndFlag > 0 {
		if *rangeStartFlag > *rangeEndFlag {
			log.Fatalf("range-start must be <= range-end")
		}
...
func fetchBlockWithRetry(ctx context.Context, client *ethclient.Client, blockNumber *big.Int, maxRetry int) (*types.Block, error) {
	var lastErr error
	for i := 0; i < maxRetry; i++ {
		...
		if i < maxRetry-1 {
			...
			log.Printf("[WRAN] failed to fetch block %s,retry %d/%d after %v:%v",
				blockNumber.String(), i+1, maxRetry, backoff, err)
			time.Sleep(backoff)
		}

	}
	return nil, fmt.Errorf("failed after %d retries: %w", maxRetry, lastErr)
}

func fetchBlockRange(ctx context.Context, client *ethclient.Client, start, end uint64, rateLimit time.Duration) {
	fmt.Printf("\n===Fecthing Block Range [%d, %d]===\n", start, end)
