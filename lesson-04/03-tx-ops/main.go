...
func main() {
	...
	if *sendMode {
		...
		// 查询交易模式
		if *txHashHex == "" {
			log.Fatalf("query mode requires --tx flag, or use --send for send mode")
		}
...
func queryTransaction(txHashHex string) {
	...
	if err != nil {
		log.Printf("failed get receipt (maybe pending):%v", err)
		return
	}
	fmt.Println("=== Receipt ===")
...
func sendTransaction(toAddrHex string, amountEth float64) {
	...
	fmt.Printf("当前连接的链 ID：%d（Sepolia 应为 11155111）\n", chainID.Uint64())
	...
	fmt.Println("Transaction is pending. Use --tx flag to query status:")
