...
func main() {
	...
	addrHex := flag.String("address", "", "address (for balaceOf or tranfer to)")
	...
	default:
		log.Fatalf("unknow mode:%s (use:balance,transfer, or parse-event)", *mode)
	}
}
...
func handleBalanceOf(ctx context.Context, client *ethclient.Client, parsedABI abi.ABI, contractHex, addrHex string) {
	...
	// 临时加的：查询原生 ETH 余额
	ethBalance, err := client.BalanceAt(ctx, targetAddr, nil)
	if err != nil {
		log.Fatalf("get ETH balance err: %v", err)
	}
	fmt.Printf("ETH Balance (wei): %s\n", ethBalance.String())
	// 换算成 ether
	etherBalance := new(big.Float).Quo(
		new(big.Float).SetInt(ethBalance),
		big.NewFloat(1e18),
	)
	fmt.Printf("ETH Balance (ether): %s\n", etherBalance.Text('f', 4))
...
func handleTransfer(ctx context.Context, client *ethclient.Client, parsedABI abi.ABI, contractHex, toHex, amountStr string) {
	...
	fmt.Printf("\n")
	// 等待交易确认
	waitForTransaction(ctx, client, signedTx.Hash(), gasLimit)
}

// waitForTransaction 等待交易确认并显示回执信息
func waitForTransaction(ctx context.Context, client *ethclient.Client, txHash common.Hash, gasLimit uint64) {
	// 设置超时上下文（最多等待 2 分钟）
	...
	for {
		...
		case <-ticker.C:
			...
			if receipt.Status == 0 {
				fmt.Printf("\n Transaction failed! Check the transaction on block explorer.\n")
			} else {
...
func handleParseEvent(ctx context.Context, client *ethclient.Client, parsedABI abi.ABI, txHashHex string) {
	...
	for _, vLog := range receipt.Logs {
		...
		fmt.Printf("  Topics[1]:from (indexed address) - 存储在Topics中\n")
		fmt.Printf("  Topics[2]:to (indexed address) - 存储在Topics中\n")
		fmt.Printf("  Data     :value(non-indexed uint256) - 存储在Data中\n")
		fmt.Printf("\n")
		fmt.Printf("为什么这样存储?\n")
		fmt.Printf("  indexed 参数:可以用于事件过滤和搜索，存储在Topics中\n")
		fmt.Printf("  non-indexed 参数:完整数据存储在Data中，使用ABI编码\n")
		fmt.Printf("  Topics 最多4个元素，因此最多3个indexed参数\n")
		fmt.Printf("----------------------------------------------------------\n")
		fmt.Printf("\n")
		...
		if len(vLog.Topics) >= 2 {
			...
			fmt.Printf("   Explanation: address类型在Topics中是32字节,前12字节为0填充\n")
			// 解析 address：去除前 12 字节的 0 填充，后 20 字节是地址
			fromAddr := common.BytesToAddress(vLog.Topics[1].Bytes()[12:])
		...
		if len(vLog.Topics) >= 3 {
			...
			fmt.Printf("  Explanation:address类型在topics中是32字节，前12字节为0填充\n")
			// 解析 address：去除前 12 字节的 0 填充，后 20 字节是地址
			toAddr := common.BytesToAddress(vLog.Topics[2].Bytes()[12:])
			fmt.Printf("  Parsed Address:%s\n", toAddr.Hex())
			fmt.Printf("\n")
		}
		...
		if len(vLog.Data) > 0 {
			...
			fmt.Printf("\n")
			fmt.Printf("Data字段包含所有non-indexed参数的ABI编码数据\n")
			fmt.Printf("对于Transfer事件，Data中只包含value(uint256)\n")
			fmt.Printf("\n")
			...
			if err != nil {
				...
				fmt.Printf("Decoded Parameter from Data:\n")
				// Transfer 事件只有一个 non-indexed 参数：value
				if len(values) > 0 {
					value, ok := values[0].(*big.Int)
					if ok {
						fmt.Printf("   value(uint256): %s\n", value.String())
						fmt.Printf("   Explanation:uint256类型直接存储在Data中，使用ABI编码\n")
					}
				}
			}
